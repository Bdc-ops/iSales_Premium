            ...
            await axios.get(`${srv}/api/index.php/categories?sortfield=t.rowid&sortorder=ASC&limit=1&page=${j}&sqlfilters=type%3D0`, { 'headers': { 'DOLAPIKEY': token, 'Accept': 'application/json' } })
                .then(async (response) => {
                    if (response.status === 200) {
                                ...
                                tx.executeSql(
                                    ...
                                    async (tx, results) => {
                                        if (results.rowsAffected > 0) {
                                            console.log('Categorie :'+mydata[index].label+' - Parent : '+mydata[index].fk_parent);
                        ...
                        j = j + 1;
                        console.log('Categorie : ' + j);
                        this.setState({index_cat:j});
                    }
                })
                .catch(async (error) => {
                            ...
                            .then(async (response) => {
                                if (response.status === 200) {
                                    ...
                                    Object.keys(mydata, ind, cat_id).forEach(async function (index, i) {
                                        ...
                                        //console.log('[Product INSERED] - page : ' + j + ' - line : ' + j + ' - categorie : ' + cat_id);
                                        //console.log('Product ============> '+mydata[index].label);
                                        
                                    });
                                    cn +=1 ;
                                    console.log('Produit : ' + cn);
                                    this.setState({index_prod:cn});
                            ...
                            <View style={styles.container}>
...

                                {
                                    ...
                                    (<View style={{alignItems: 'center',justifyContent: 'center',}}>
                                        ...
                                        <Text style={styles.loading_text}>{this.state.index_cat} Categories téléchargées</Text>
                                        <Text style={styles.loading_text}>{this.state.index_prod} Produits téléchargées</Text><Text></Text><Spinner /></View>)
                                        }
