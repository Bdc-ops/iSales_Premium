...
export default class SearchCategoriesProduits extends React.Component {
  ...
  componentDidMount() {
    let log_msg1 = "\n" + moment().format('YYYY-MM-DD hh:mm:ss a') + " : Recherche des produits (fenetre categories)\n-------";
  ...
  closeDB(db) {
    ...
    let log_msg7 = "\n" + moment().format('YYYY-MM-DD hh:mm:ss a') + " : Recherche produits (categories): Couper la connexion avec la base de donnee \n-------";
  ...
  search_categories() {
    ...
    let log_msg2 = "\n" + moment().format('YYYY-MM-DD hh:mm:ss a') + " : Recherche produits (categories): recherche categorie :" + rechercheinput + "\n-------";
    RNFS.appendFile(RNFS.ExternalStorageDirectoryPath + '/iSales_Premium/Backups/logs/logs.txt', log_msg2, 'utf8');


    db.transaction(tx => {
      tx.executeSql(`SELECT c.id_categorie,c.label,(select categories.label from categories where categories.id_categorie=c.fk_parent) as parent,(SELECT count(*) FROM produits where produits.id_categorie=c.id_categorie) as count from categories c where c.label LIKE '${rechercheinput}%' OR c.label LIKE '%${rechercheinput}' OR c.label LIKE '%${rechercheinput}%' OR c.label LIKE '${rechercheinput}'`, [], (tx, results) => {
        var temp = [];
        for (let i = 0; i < results.rows.length; ++i) {
          temp.push(results.rows.item(i));
        }
  ...
  search_produits() {
    ...
    let log_msg3 = "\n" + moment().format('YYYY-MM-DD hh:mm:ss a') + " : Recherche produits (categories): recherche tous les produits - produit: " + rechercheinput + "\n-------";
  ...
  search_produit_cible() {
    ...
    let log_msg4 = "\n" + moment().format('YYYY-MM-DD hh:mm:ss a') + " : Recherche produits (categories): recherche produit cible - produit: " + rechercheinput + "\n-------";
  ...
  _ShowCategorie(index) {
    ...
    let log_msg5 = "\n" + moment().format('YYYY-MM-DD hh:mm:ss a') + " : Recherche produits (categories): ouverture categorie :" + index + "\n-------";
  ...
  _Showinfos(ref, rechercheinput) {
    this.props.navigation.navigate('ShowProduitInfos', { ref: ref, rechercheinput: rechercheinput, searchsource: 1 });
    let log_msg6 = "\n" + moment().format('YYYY-MM-DD hh:mm:ss a') + " : Recherche produits (categories): ouverture produit ref :" + ref + " - label :" + rechercheinput + "\n-------";
          ...
          if (state.searchframe === 1) {
            return (
              ...
              (
                ...
                <CardView key={index} cardElevation={5} cornerRadius={25} style={styles.cardViewStyle0}>
                  <TouchableOpacity style={styles.cardViewStyle0} onPress={() => rowData.count > 0 ? this._ShowCategorie(rowData.id_categorie) : console.log('La categorie est vide')}>
                    <View style={styles.details}>
                      {/*<View style={styles.cat_name}>
                        <Icon name="box" size={20} style={styles.iconDetails} />
                        <Text style={styles.categoriesname}>{rowData.label}</Text>
                      </View>*/}
                      <View style={styles.cat_name}>
            ...
            <View style={styles.cardViewStyle}>
              <CardView cardElevation={10} cornerRadius={5} style={styles.cardViewStyle1}>
                ...
                <View style={styles.SearchContainer}>
                  <TextInput style={styles.inputs} placeholder="Nom/Réf du produit ou Nom de la catégorie"
                  ...
                  <View style={styles.SearchButtons}>
                    <TouchableOpacity style={styles.SubmitButtonStyle} activeOpacity={.5}
                      onPress={this.search_categories.bind(this)}>
                      <Icon name="search" size={10} style={styles.iconSearch} />
                      <Text style={styles.TextSearch}>Categories</Text>
                    </TouchableOpacity>

                    <TouchableOpacity style={styles.SubmitButtonStyle} activeOpacity={.5}
                      onPress={this.search_produits.bind(this)}>
                      <Icon name="search" size={10} style={styles.iconSearch} />
                      <Text style={styles.TextSearch}>Produits</Text>
                    </TouchableOpacity>
