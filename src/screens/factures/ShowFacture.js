...
import Show_factures_navbar from '../../navbars/factures/show_factures_navbar';
import CardView from 'react-native-cardview';
import { openDatabase } from 'react-native-sqlite-storage';
var db = openDatabase({ name: 'iSalesDatabase.db' });
import moment from 'moment';
const avatar_image = require('../../res/factures.png');
const IMG2 = require('../../res/loading.png');
import Icon from 'react-native-vector-icons/FontAwesome5';


export default class ShowFacture extends React.Component {
    constructor(props) {
        super(props);
    ...
    componentDidMount() {
        ...
        console.log('#######################');
        db.transaction(tx => {
            tx.executeSql(`SELECT ff.*,(SELECT count(*) FROM lines_facture where lines_facture.id_facture=ff.id) as count,(SELECT name from clients where clients.ref=ff.socid) as clientname from factures ff where ff.id=${this.state.data}`, [], (tx, results) => {
                var temp = [];
                let cond_reglement ='';
                let mode_reglement='';
                ...
                console.log('COND : '+results.rows.item(0).cond_reglement_code+' - MODE : '+results.rows.item(0).mode_reglement);
                ...
                tx.executeSql(`SELECT * from lines_facture where id_facture=${this.state.data}`, [], (tx, results) => {
                    var temp = [];
                    for (let i = 0; i < results.rows.length; ++i) {
                        temp.push(results.rows.item(i));
                        console.log(results.rows.item(i));
    ...
    render() {
        ...
        return (
            ...
            <ScrollView ref='_scrollView' contentContainerStyle={styles.containerMain}>
                <Show_factures_navbar title={navigate}></Show_factures_navbar>
                        ...
                        state.bg_timer === null ?
                            ...
                            (<View style={styles.containerResults}>
                                ...
                                <CardView cardElevation={10} cornerRadius={5} style={styles.cardViewStyle1}>
                                    <Text style={styles.detailslabel}>Details de la facture</Text>
                                    {
                                        state.facture_client.length > 0 ?
                                            state.facture_client.map((clientRow, j) => (
                                                <View key={j} style={styles.ClientContainer} key={j}>
                                                    ...
                                                    <View style={{ width: '60%' }}>
                                                        ...
                                                        <View style={{ flexDirection: 'row' }}><Text>Client :</Text><Text style={styles.text_client} numberOfLines={2}> {clientRow.clientname ? clientRow.clientname : '- - -'}</Text></View>
                                                        <View style={{ flexDirection: 'row' }}><Text>Ref facture :</Text><Text style={styles.texts}> {clientRow.ref && clientRow.ref != 0 ? clientRow.ref : clientRow.ref === '0' ? 'Nouvelle commande' : '- - -'}</Text></View>
                                                        <View style={{ flexDirection: 'row' }}><Text>Date création :</Text><Text style={styles.texts}>

                                                            {//moment(new Date(clientRow.date_creation * 1000)).format('DD-MM-YYYY') ?moment(new Date(clientRow.date_creation * 1000)).format('DD-MM-YYYY')
                                                        ...
                                                        <View style={{ flexDirection: 'row' }}><Text>Date facturation :</Text><Text style={styles.texts}>

                                                            {
                                                                //moment(new Date(clientRow.date * 1000)).format('DD-MM-YYYY') ?moment(new Date(clientRow.date * 1000)).format('DD-MM-YYYY')
                                                                clientRow.date?
                                                                moment(new Date(clientRow.date * 1000)).format('DD-MM-YYYY')
                                                                :
                                                                '----'
                                                            }
                                                        </Text></View>
                                                        ...
                                                        <View style={{ flexDirection: 'row' }}><Text>Etat :</Text><Text style={styles.texts}> {clientRow.etat === '1' ? 'Impayée' : 'Payée'}</Text></View>
                                ...
                                <CardView cardElevation={10} cornerRadius={5} style={styles.cardViewStyle}>
                                    ...
                                    <View style={styles.listeproduits}>
                                        ...
                                        {
                                            state.facture_produits.length > 0 ?
                                                state.facture_produits.map((rowData, index) => (
                                                    rowData.is_jalon == 0 ?
                                                    (
                                                        /* Product normale */
                                                    <View key={index} style={styles.detailsproduits}>
                                                        ...
                                                        <View style={styles.details}>
                                                            <View style={styles.productheader}>
                                                                <Text style={styles.productname}>{rowData.libelle?rowData.libelle:'- - -'} </Text>
                                                            </View>
                                                            <View style={styles.productinfos}>
                                                                <Text style={styles.TarifText}>Prix de vente : </Text>
                                                                <Text style={styles.TarifText}>{(parseFloat(rowData.subprice)).toFixed(2)} €</Text>
                                                            </View>
                                                            {/*<View style={styles.productinfos}>
                                                                <Text style={styles.TarifText}>Prix d'unité : </Text>
                                                                <Text style={styles.TarifText}>{(parseFloat(rowData.total_ht / rowData.qty)).toFixed(2)} €</Text>
                                                            </View>
                                                            <View style={styles.productinfos}>
                                                                <Text style={styles.TarifText}>Prix d'achat : {parseFloat(rowData.pa_ht).toFixed(2)} €</Text>
                                                            </View>*/}
                                                            {

                                                                <View style={styles.productinfos}>
                                                                    <Text style={styles.TarifText}>TVA : {(parseFloat(rowData.tva_tx)).toFixed(2)} %</Text>
                                                                </View>
                                                        ...
                                                        <View style={styles.pricedetails}>
                                                            <Text style={styles.price}>{parseFloat(rowData.total_ht).toFixed(2)}</Text>
                                                        </View>
                                                    </View>
                                                    )
                                                    :
                                                    ( /* JALON */
                                                    <View key={index} style={styles.detailsjalon}>
                                                        <Icon style={{padding:20,color:'#00BFA6'}} name="info-circle" size={20}/>

                                                            <View style={styles.jalonheader}>
                                                                <Text>{rowData.libelle?(<Text>{rowData.libelle} <Text style={{color:'#00BFA6'}}> (Jalon)</Text></Text>):'- - -'} </Text>
                                                            </View>
                                                    </View>
                                                    )
                                                ))
                                        ...
                                        {
                                            state.facture_client.map((rowData, index) => (
